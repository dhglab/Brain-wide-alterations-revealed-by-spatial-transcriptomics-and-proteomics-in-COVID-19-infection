...
setwd("./Fig2Sourcedata/WGCNA/concensusModule")
...
for(minModuleSize in c(50, 100, 150)){
  for(ds in c(2,4)){
    for(dthresh in c(0.1, 0.2, 0.25)){
      print("trying parameters:")
      print(c(minModuleSize, dthresh,ds))
      tree <- cutreeHybrid(dendro = geneTree, distM = as.matrix(dissTOM),
                           deepSplit = ds, pamRespectsDendro = FALSE,pamStage = FALSE,
                           minClusterSize = minModuleSize, cutHeight = 0.9999)
      merged <- mergeCloseModules(exprData = multiExpr,colors = tree$labels,
                                  cutHeight = dthresh)
      mColorh <- cbind(mColorh, labels2colors(merged$colors))
      mLabelh <- c(mLabelh, paste("DS=", ds, "mms=\n", minModuleSize, "dcor=", dthresh))
    }
  }
}

pdf("consensusTOMFrontalGM_PN.pdf", width = 12, height = 10)
plotDendroAndColors(geneTree, mColorh, groupLabels = mLabelh,
                    main = "Dendrogram with different module cutting thresholds",dendroLabels = FALSE,
                    hang = 0.03, addGuide = TRUE, guideHang = 0.05)
dev.off()

modules = mColorh[,4] # (Chosen based on plot)
...
save(geneTree, mColorh, modules, tree, file = "FrontalGM and PN concensus_module_choice.RData")
...
consensus_kM_s <- consensus_kM[, grep("consensus", colnames(concensus_kM))]
...
#Use the concensus modules to calculate eigengenes#
...
save(geneTree, mColorh, modules, tree,FrontalGMQ3_reg, target_Frontal, PNQ3_reg, target_PN, MEList_Fc, MEs_Fc, MEList_Pc, MEs_Pc, file = "FrontalGM and PN concensus_module_choice.RData")
...
save(Res_filtered_Fc,Res_filtered_grey_Fc, lm_results_Fc, ME_names_Fc, New_Fc, file = "DifferentialFrontalGMmodules_Concensus.RData")
