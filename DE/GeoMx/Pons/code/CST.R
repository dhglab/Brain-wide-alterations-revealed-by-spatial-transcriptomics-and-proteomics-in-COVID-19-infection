...
#Load raw(QC_remove non integer), targets, and differential expression from GeoDSP profiler, CSTFDRQC, CST4_6_DEG, CSTQC, TargetCST

#check rownames of target and colnames of matrix
all(rownames(TargetCST) == colnames(CSTFDRQC)) 

dds <- DESeqDataSetFromMatrix(countData = CSTFDRQC,
                              colData = TargetCST,
                              design = ~ Disease)
dds
...
pca <- pca(mat, metadata = TargetCST)

#Supplementary Figure 1A and 1B
tiff("CST5+6_PCA.tiff", units="in", width= 6.5, height=4, res=300)
...
tiff("CST4+6_correlates.tiff", units="in", width= 6, height=3.5, res=300)
eigencorplot(pca,
             ...
             rotLabX = 45,
             scale = TRUE,
             main = bquote(CST ~ PCA ~ Pearson ~ r^2 ~ clinical ~ correlates),
             cexMain = 1.5,
             plotRsquared = TRUE,
             corFUN = 'pearson',
