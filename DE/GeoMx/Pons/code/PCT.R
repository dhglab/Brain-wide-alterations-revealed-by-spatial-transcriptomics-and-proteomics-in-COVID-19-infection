...
#Load raw(QC_remove non integer), targets, and differential expression from GeoDSP profiler,PCT4_6_DEG, PCTFDR01, PCTFDRQC, TargetPCT

#check rownames of target and colnames of matrix
all(rownames(TargetPCT) == colnames(PCTFDRQC)) 

dds <- DESeqDataSetFromMatrix(countData = PCTFDRQC,
                              colData = TargetPCT,
                              design = ~ Disease)
dds
...
pca <- pca(mat, metadata = TargetPCT)


#Supplementary Figure 1A and 1B
tiff("PCT4+6_PCA.tiff", units="in", width= 6.5, height=4, res=300)
biplot(pca, x = 'PC1', y = 'PC2',
       ...
       title = "Pontocerebellar Tract")
dev.off()

tiff("PCT4+6_correlates.tiff", units="in", width= 6, height=3.5, res=300)
eigencorplot(pca,
             ...
             rotLabX = 45,
             scale = TRUE,
             main = bquote(PCT ~ PCA ~ Pearson ~ r^2 ~ clinical ~ correlates),
             cexMain = 1.5,
             plotRsquared = TRUE,
             corFUN = 'pearson',
