...
#Load raw(QC_remove non integer), targets, and differential expression from GeoDSP profiler, TegFDRQC,TegQC, TargetTeg, Teg4_6_DEG

#check rownames of target and colnames of matrix
all(rownames(TargetTeg) == colnames(TegFDRQC)) 

dds <- DESeqDataSetFromMatrix(countData = TegFDRQC,
                              colData = TargetTeg,
                              design = ~ Disease)
dds
...
pca <- pca(mat, metadata = TargetTeg)

#Supplementary Figure 1A and 1B
tiff("Teg5+6_PCA.tiff", units="in", width= 6.5, height=4, res=300)
...
tiff("Teg4+6_correlates.tiff", units="in", width= 6, height=3.5, res=300)
eigencorplot(pca,
             ...
             rotLabX = 45,
             scale = TRUE,
             main = bquote(Teg ~ PCA ~ Pearson ~ r^2 ~ clinical ~ correlates),
             cexMain = 1.5,
             plotRsquared = TRUE,
             corFUN = 'pearson',
